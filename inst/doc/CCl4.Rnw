...
\title{From Genepix data file to RGList to NChannelSet}
...
We locate it on your filesystem with the function \Rfunction{system.file}.

<<RGList, results=hide>>=
datapath <- system.file("extdata", package="CCl4")
p <- read.table(file.path(datapath, "samplesInfo.txt"), sep="\t", 
                row.names=1L, as.is=TRUE, header=TRUE)


CCl4_RGList <- read.maimages(files=rownames(p), 
   path = datapath, 
   source = "genepix", 
...
varMetadata <- data.frame(labelDescription=character(ncol(p)),
                              channel=factor(rep("_ALL_", ncol(p))))
